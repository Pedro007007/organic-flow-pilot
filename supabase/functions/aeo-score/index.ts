...
serve(async (req) => {
  ...
  try {
    ...
    if (!content.trim()) {
      return new Response(
        JSON.stringify({ error: "No draft content to score" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
            ...
            },
            {
              ...
              content:
                `Analyze this content for AEO readiness. Schema types present: ${
                  JSON.stringify(item.schema_types || [])
                }.\n\nContent:\n${analysisWindow}`,
            },
          ],
          tools: [
                ...
                parameters: {
                  ...
                  properties: {
                    ...
                    entity_clarity: {
                      type: "integer",
                      description:
                        "0-100: Are key entities explicitly defined?",
                    },
                    schema_richness: {
                      type: "integer",
                      description:
                        "0-100: FAQPage, HowTo, QAPage schemas present?",
                    },
                    conciseness: {
                      type: "integer",
                      description:
    ...
    await supabase.from("content_items").update({ seo_score: overall }).eq(
      "id",
      contentItemId,
    );
